...
lb_df["weight"] = (lb_df["weight"].astype(float) * 0.453592).round().astype(int).astype(str) + " lb"
...
in_df["height"] = (in_df["height"].astype(float) * 0.393701).round().astype(int).astype(str) + " in"

#Adding converted values into the each df matching by id
no_nan_weights_df.loc[no_nan_weights_df["id"].isin(lb_df["id"]), "weight"] = lb_df["weight"]
no_nan_heights_df.loc[no_nan_heights_df["id"].isin(in_df["id"]), "height"] = in_df["height"]
