...
def scrape_data_for_year(year, division = "mens"):
    ...
    scores_df = pd.DataFrame(scores_data, columns=["id"]+year_to_col_name_mapping[year])



    # save to csv
    athlete_info_df.to_csv(f'{year}_{division}_info.csv')
    scores_df.to_csv(f'{year}_{division}_scores.csv')
