...
def retrieve_benchmark_info(athlete_id):
    ...
    try:
        ...
        benchmark_stats_container = soup.find('div', id="benchmarkStats").find('ul', class_="stats-container")
    except AttributeError:
            return f"An error occurred when trying to retrieve athlete id: {athlete_id} benchmark statistics."
    else:
        list = [f"athlete_id: {athlete_id}", f"name: {name}"]
        stats_section = benchmark_stats_container.find_all('div', class_="stats-section")
