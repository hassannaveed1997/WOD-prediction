...
def convert_time_cap_workout_to_time(x: str, total_reps: int, time_cap:int , scale_up=False):
    """
    Here x can either be in reps or a time. For example, if the workout had a total of 100 reps and a time cap of 20 minutes,
    ...
    try:
        if x in [np.nan, 'nan', pd.NaT]: # if missing, return nat timedelta
        ...
        elif ":" in x:# if x is reported as a time, athlete finished. Hacky Solution: Looking for ":" in the string to determine if it is a time    
            ...
            time_cap = time_delta_format[:len(time_delta_format) - len(time_cap)] + time_cap
        ...
        warnings.warn(f"Could not convert {x_orig} to time, returning as is. Error: {e}")


def convert_to_floats(df: pd.DataFrame, descriptions: dict, threshold = .5, scale_up = False):
    """
    ...
    for workout_name in df_modified.columns:
        ...
        workout_name_base = workout_name.replace("_scaled", "").replace("_foundation", "")

        # if we don't have the workout name in descriptions, we skip it
        if workout_name_base not in descriptions:
            warnings.warn(f"Workout {workout_name} not found in descriptions, skipping preprocessing. Please inspect manually")
            continue

        # if workout is for REPS, we should be fine
        if descriptions[workout_name_base]["goal"].lower() in ["reps", "amrap"]:
            df_modified[workout_name] = df_modified[workout_name].astype(int, errors='ignore')
            ...
            if descriptions[workout_name_base]["time_cap"] is not None:
                ...
                raise ValueError(f"Workout {workout_name} is for time, but does not have a time cap. Please inspect descriptions manually")
            ...
            raise ValueError(f"Workout {workout_name} has an unknown goal. Please inspect descriptions manually")
        
        # manual inspection
        try:
            pd.to_numeric(df_modified[workout_name])
        except ValueError:
            warnings.warn(f"Could not convert column {workout_name} to float. Please inspect manually")
